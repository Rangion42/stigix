...
"""
    ...
    parser.add_argument(
        '--tcp-src-port',
        type=int,
        help='TCP source port to filter'
    )

    parser.add_argument(
        '--tcp-dst-port',
        type=int,
        help='TCP destination port to filter'
    )
...
def main():
    ...
    elif args.tcp_src_port or args.tcp_dst_port:
        log_output(" - Protocol: TCP", json_mode)

    if args.udp_src_port:
        log_output(f" - Source Port (UDP): {args.udp_src_port}", json_mode)

    if args.udp_dst_port:
        log_output(f" - Destination Port (UDP): {args.udp_dst_port}", json_mode)

    if args.tcp_src_port:
        log_output(f" - Source Port (TCP): {args.tcp_src_port}", json_mode)

    if args.tcp_dst_port:
        log_output(f" - Destination Port (TCP): {args.tcp_dst_port}", json_mode)
    ...
    if args.protocol:
        ...
        query_payload["filter"]["flow"]["protocol"] = 6

    if args.udp_src_port or args.tcp_src_port:
        port_val = args.udp_src_port if args.udp_src_port else args.tcp_src_port
        query_payload["filter"]["flow"]["source_port"] = [port_val]

    if args.udp_dst_port or args.tcp_dst_port:
        port_val = args.udp_dst_port if args.udp_dst_port else args.tcp_dst_port
        query_payload["filter"]["flow"]["destination_port"] = [port_val]
